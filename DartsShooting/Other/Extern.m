//
//  Extern.m
//  DartsShooting
//
//  Created by 田伟 on 2018/5/14.
//  Copyright © 2018年 MG. All rights reserved.
//

#import "Extern.h"

uint32_t const WordCategory = 0x1 << 0;
uint32_t const FallBallCategory = 0x1 << 1;
uint32_t const FixedBallCategory = 0x1 << 2;
uint32_t const FixedBallOneCategory = 0x1 << 3;
uint32_t const FixedBallTwoCategory = 0x1 << 4;
uint32_t const FixedBallThreeCategory = 0x1 << 5;


NSInteger const Referenceszposition = 1;// 参照点
NSInteger const Treebgzposition = 10;//转盘背景
NSInteger const Knifezposition = 20;// 刀
NSInteger const TreeBoundzposition = 30;// 转盘
